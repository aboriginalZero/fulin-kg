...
3. 本地感知
    ...
    Oracle RAC 等小范围共享卷的场景可能会有 2 ～ 3 个活跃接入点，但处理方式不变。特别地，有些场景如 Xen 平台的 iSCSI DataStore 模式将一个 volume 作为一个池使用，不同节点上的虚拟机仅访问其中的部分数据，卷的活跃接入点可能会超过 3 个，此时持有的 pextent 只会在初次写入时用第一个接入节点作为 prefer local，后续不再变更。
...
在 ZBS 集群运行过程中，当集群中没有待恢复数据块时，不论集群此时处于何种负载，meta leader 的一个后台线程会周期性地扫描所有副本/节点，按某种迁移策略选出待迁移数据块的 src / dst / replace cid 生成迁移命令，并为其选定/分配 lease 后，通过心跳下发给 access 执行。
...
3. 高负载时，已经放弃了本地优先原则，所以若集群没有配置拓扑信息将直接执行节点容量均衡迁移扫描。
...
局部化迁移（migrate for prefer local）指的是：当数据块的 prefer local 不超过严格中负载并且健康、不是 isolated 时，将使其拥有分片以提高数据访问性能。
...
除了容量上的限制，根据 replace cid 的负载不同，可允许迁移的 pextent 类型也不同：

* 中负载：不允许迁移作为 parent 的 pextent、prefer local 上的 pextent、lease owner 上的 pextent；
* 高负载：不允许迁移 lease owner 上的 pextent；
* 超高负载：所有 pextent 都可能被迁移，另外此时还允许 src cid 是 isoalted（但优先级是最低的）。
...
一般来说，集群中的均匀卷数量较少，所以不关注负载情况，只会尝试触发以 ① 拓扑修复 ② 数量均衡为目的的迁移。均匀卷的拓扑修复与非均匀卷策略一致，不再赘述。其中，数量均衡指的是所有均匀卷的数据块分片需要相对均匀地分布在整个集群，每个节点持有的均匀块数量大致相等。
...
由于节点移除迁移命令的 replace cid 一定是待移除节点，且节点移除需要尽快完成，所以对 dst cid 没有拓扑上的严格要求，基本可以复用分片恢复策略中的选取规则。
...
在空间不足时，即使 topo 降级也可以迁出



## 分片恢复策略

如果有分片待恢复，且其 migrate cmd 还存在，会先取消
